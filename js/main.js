...
function createGrid(){
    ...
    for (let row = 0; row <= 3; row++){
        for(let col = 0; col <= 3; col++){
            // respectively build new grid in the container, and give id&& class
...
function initGrid() {
    // initialize grid in the document 
    score = 0; 

    let gameScore = document.querySelector(".current-score");
    gameScore.innerHTML = "0";
    console.log(gameScore.innerHTML);

    for(let row = 0; row <= 3; row++){
        // each time focus on one row
        board[row] = new Array();
        for(let col = 0; col <= 3; col++){
            board[row][col] = 0;
...
function updateBoardView() {
    // each time update the board, firstly remove all the number cell class
    $(".number-cell").remove();
    for(let row = 0; row <= 3; row++){
